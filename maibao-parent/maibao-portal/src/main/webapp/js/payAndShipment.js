...
function doDealCalenderInfo(dataResult) {
	...
	if (dataResult.suportShipType == "1") { //如果只支持311
	    ...
	    if (dataResult.suportShipType == "0") {
	      $("#jdShip-span-tip .qmark-icon").attr('data-tips', '由买宝公司负责配送，速度很快，还接受上门刷卡付款服务')
	    } else {
...
function doSaveInstallDate(venderId) {
  ...
  $("#install_date_div_" + venderId).html('<span class="ftx-03">安装时间：</span>' + installDate_date + " " + installDate_weekDay + '<span class="ftx-03 ml5">只针对买宝上门安装商品</span>');
...
function doSaveOtherInstallDate(venderId){
   ...
   $("#otherinstall_date_div_" + venderId).html('<span class="ftx-03">安装时间：</span>' + installDate_date + " " + installDate_weekDay +'<span class="ftx-03 ml5">只针对买宝上门安装商品</span>');
