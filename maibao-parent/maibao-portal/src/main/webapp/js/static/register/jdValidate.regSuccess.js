...
(function () {
    ...
    $('#emailStr').focus(function () {
        ...
        $("#email_error").html("");
        $("#email_focus").html("完成验证后，您可以用该邮箱登录买宝，找回密码。");
    });
    ...
    $('#moblie').bind('focus', function () {
        ...
        $("#smsFocusDiv").removeClass().addClass("item");
        $("#smsFocusMessage").removeClass().addClass("sms-tips mobileFocus").text("完成验证后，您可以用该手机号登录买宝，找回密码。");
    });
...
function reSendEmail(email, key) {
    ...
    $.getJSON("../notifyuser/email?email=" + (email) + "&k=" + key + "&state=" + unbind+ "&r=" + Math.random(), function (result) {
        ...
        if (result.success == -4) {
            $('#reSendEmailSuccess').removeClass().addClass('error').empty().html('该邮箱已注册过买宝');
            $('#reSendEmailSuccess').show();
            return;
        }
