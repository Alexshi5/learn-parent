...
//            }
//            $("<div class='dt'>服\u3000\u3000务：</div><div class='dd'>由 买宝 发货并提供售后服务"+upenCheckStr+"。"+iconDesc+"</div>").appendTo("#summary-service");
//        }
    }
    if (pageConfig.product.skuid>1000000000){   
        if (r){
...
//                var shinfo=r.po=="false"?"买宝提供售后服务。":"并提供售后服务。"; //sop & sopl
//				if(pageConfig.product.isFlashPurchase){
//					if(r.type==0){
//						requestDeliveCash = true;
//						getPOPDeliveCash();
//					}
//					$("<div class='dt'>服\u3000\u3000务：</div><div class='dd'>由 买宝 负责发货，<a href='#none'>"+r.vender+"</a> 完成售后服务。"+iconDesc+"</div>").appendTo("#summary-service");
//				}
//				else{
//					if(r.type==0){
...
//					}
//					else if(r.type==1){
//						$("<div class='dt'>服\u3000\u3000务：</div><div class='dd'>由 买宝 发货并提供售后服务。"+iconDesc+"</div>").appendTo("#summary-service");
//					}
//					else if(r.type==2){
//						$("<div class='dt'>服\u3000\u3000务：</div><div class='dd'>由<a href='"+$.getShopUrl(r)+"' "+(pageConfig.product.isFlashPurchase?"":"target='_blank'")+" clstag='shangpin|keycount|product|bbtn' class='hl_red'>"+r.vender+"</a>"+dfinfo+"发货，买宝提供售后服务。"+iconDesc+"</div>").appendTo("#summary-service");
//					}
//					else if(r.type==5){
...
function getTJAreaSku(r){
    ...
    if(r&&r.resultCode==1&&r.skuId&&r.skuId!=pageConfig.product.skuid+''){
		...
		if((r.skuId+"").length==10){
			...
			+r.skuId+'.html" clstag="shangpin|keycount|product|yipinduoshang">买宝商城</a></div><div class="lh hl_red"></div></li></ul></div>').appendTo("#ypds-list");
			$.ajax({
				url:"http://p.3.cn/prices/get?skuid=J_"+r.skuId+"&type=1&area="+currentAreaInfo.currentProvinceId+"_"+currentAreaInfo.currentCityId+"_"+currentAreaInfo.currentAreaId,
				dataType:"jsonp",
				type:"get",
